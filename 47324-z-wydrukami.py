...
def mutuj(xp, pm):
    for i, chromosom in enumerate(xp, 1):
        for j, gen in enumerate(chromosom):
            ...
            if p < pm:
                print(
                    f"  g{j+1}: pm = {p:.2f} < {pm:.2f}, zachodzi mutacja {chromosom[j]} --> {chromosom[j] ^ 1}"
                )
                chromosom[j] ^= 1
            else:
                print(f"  g{j+1}: pm = {p:.2f}, bez mutacji")
    ...
    for i in range(0, len(xp) - 1, 2):
        ...
        if p <= pk:
            if lg > 1:
                ...
                print(f"{p}<={pk} (p<=pk), zachodzi krzyzowanie miedzy genami {punkt_ciecia} a {punkt_ciecia+1}")
                print(f"Potomstwo: {xp_kopia[i]} i {xp_kopia[i+1]}")
            else:
                xp_kopia[i], xp_kopia[i + 1] = xp[i], xp[i + 1]
        else:
            print(f"{p}>{pk} (p>pk), brak krzyzowania.")

    return xp_kopia


def wydruki(xp, ocena_populacji, suma_wag_chromosomow, wartosci, wagi, waga_max, iteracja):
    ...
    for i, (gen, ocena_ch, waga_ch) in enumerate(zip(xp, ocena_populacji, suma_wag_chromosomow), 1):
        print(f"ch {i:02}: {gen}, fp={ocena_ch}, waga={waga_ch}")

    print(f"\n### Selekcja turniejowa Populacji {iteracja}: ###\n")
    print("Turnieje: ")
...
if __name__ == "__main__":
    ...
    for i in range(1, lpop + 1):
        ...
        wydruki(xp, ocena_populacji, suma_wag_chromosomow, wartosci, wagi, waga_max, i)

        # Rodzicoe
        indeksy_rodzicow = rodzice(
            xp, ocena_populacji, suma_wag_chromosomow, waga_max)
        xp = xp[indeksy_rodzicow]
        print(f"Wygrane indeksy rodzicow: {[indeks+1 for indeks in indeksy_rodzicow]}")
    ...
    print(f"\nKoniec.\n\nNajlepszy dopuszczalny chromosom ze wszystkich populacji: ")
